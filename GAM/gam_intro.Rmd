...
# simulation
set.seed(1234)
dat = gamSim(eg = 1, n = 100, dist = "normal", scale = 2, verbose = FALSE)
...
.pull-right[
__R2:__ `r round(summary(lm(y ~ x, data = dat))$r.squared, 2)`
__AIC:__ `r AIC(lm(y ~ x, data = dat))`
]
...
.pull-right[
__R2:__ `r round(with(summary(glm(y ~ x, data = dat, family = 'Gamma')), 1 - deviance/null.deviance), 2)`
__AIC:__ `r round(AIC(glm(y ~ x, data = dat, family = 'Gamma')))`
]
...
.pull-right[
3rd order |
__R2:__  `r round(summary(lm(y ~ poly(x, 3), data = dat))$r.squared, 2)`
__AIC:__ `r round(AIC(lm(y ~ poly(x, 3), data = dat)))`

9th order |
__R2:__ `r round(summary(lm(y ~ poly(x, 9), data = dat))$r.squared, 2)`
__AIC:__ `r round(AIC(lm(y ~ poly(x, 9), data = dat)))`
]
...
.pull-right[
__Dev. explained:__ `r round(summary(gam(y ~ s(x), data = dat))$dev.expl, 2)`

__AIC:__ `r round(AIC(gam(y ~ s(x), data = dat)))`
]
...
- $b_{j,k}$ - $K$ Basis functions for each covariate $x_j$

- $\beta_{j,k}$ - $K$ Coefficients for each covariate $x_j$
...
- Predictor replaced by $K$ synthetic predictors (Basis Functions)

- Basis Functions are Weighted by Coefficients

- Summed to build Regression Line __(Spline)__

- Wiggliness is Penalized

  - Minimize Sums of Squared Error (SSE)
  
  - Minimize Wiggliness (Overfitting)
  
$$Fit = Likelihood - \lambda \times Wiggliness$$
$$\hat{\beta} = arg\,min_\beta || y-X\beta||^2 + \lambda \beta^TS\beta$$
...
```{r eval=FALSE}
predict(air1, type = 'lpmatrix') # or: model.matrix(air1)
```

```{r echo=FALSE, fig.height=4}
# plot: model matrix
ggmatplot(lpm2, x_axis_column = 1) +
  labs(title = NULL, y = 'Ozone', x = 'Wind')
```
...
Summed to build Regression Line (Spline)
...
SOLUTION: Penalize departure from smoothness:
...
sp1 = gam(y ~ s(x, k = 100),
          data = dat,
          sp = 0)
dat$pr1 = predict(sp1, type = 'response')
gg_sp1 = ggplot(dat, aes(x = x)) +
  geom_point(aes(y = y)) +
...
sp2 = gam(y ~ s(x, k = 100),
          data = dat,
          sp = 1e5)
dat$pr2 = predict(sp2, type = 'response')
gg_sp2 = ggplot(dat, aes(x = x)) +
  geom_point(aes(y = y)) +
...
sp3 = gam(y ~ s(x, k = 100),
          data = dat,
          method = 'REML')
dat$pr3 = predict(sp3, type = 'response')
gg_sp3 = ggplot(dat, aes(x = x)) +
  geom_point(aes(y = y)) +
...
mGI = gam(value_fin ~
            ...
            s(site_id_f, bs = 're'), # explicitly needed
          data = dat,
          family = Gamma(link = 'log'))
```
...
---
layout: false
# [Moving Beyond Linearity](https://andschar.github.io/teaching/GAM-intro.html)
...
<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/4f/Twitter-logo.svg/500px-Twitter-logo.svg.png" width="20"> @andschar <br/>
<img src="https://seeklogo.com/images/M/mail-icon-logo-28FE0635D0-seeklogo.com.png" width="20"> andschar@proton.me
...
<!-- knots = data.frame(x = seq(min(dat$x), max(dat$x), length.out = k)) -->
<!-- spl = smoothCon(s(x, bs = 'cr', k = k), -->
<!--                 data = dat,#[ ,'x', drop = FALSE], -->
<!--                 knots = knots)[[1]] -->
<!-- coefs = coef(gam(y ~ s(x, bs = 'cr', k = k), data = dat)) -->
<!-- fx = spl$X %*% coefs -->
...
<!-- # 1 -->
<!-- plot(fx ~ dat$x, type = "n", ylim = ylim, ylab = "", xlab = "x") -->
<!-- abline(v = knots$x, lty = "dotted") -->
<!-- matlines(dat$x, bs1, lwd = 2) -->
<!-- title(ylab = expression(f(x)), line = 2.75) -->
<!-- # 2 -->
<!-- plot(fx ~ dat$x, type = "n", ylim = ylim, ylab = "", xlab = "x") -->
<!-- abline(v = knots$x, lty = "dotted") -->
<!-- matlines(dat$x, bs_coef, lwd = 2) -->
<!-- lines(fx ~ dat$x, lwd = 4) -->
<!-- title(ylab = expression(f(x)), line = 2.75) -->
<!-- par(op) -->
<!-- layout(1) -->
<!-- ``` -->
